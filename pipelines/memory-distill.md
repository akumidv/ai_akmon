...
1. **CAPTURE** — write **one fact = one file** to `_aitna/memory/`, indexed in
   `_aitna/memory/MEMORY.md`. Concise, operational wording. This is shared project memory:
   in git, visible to all developers and agents.
   - Distinguish from **provider-private memory** (each assistant's own store). Private
     memory stays with the individual; distill *from* it into `_aitna/memory/` — never
...
2. **DISTILL** — when a fact recurs or proves load-bearing, turn it into something
   reusable:
   - a repeated procedure → a **LOCAL skill** (`_aitna/skills/<name>/SKILL.md`),
   - a repeated mechanical operation → a **LOCAL tool** (`_aitna/tools/<name>/`),
   - a way a role should work here → the project **agent charter** (`_aitna/agents/`),
...
memory-distill stays **LOCAL**. Lifting a local asset into akmon (cross-project) is
**PROMOTE**, step 3 — handled by [learning](learning.md). Do not promote here; a fact must
prove general *and* reused first.

## Done

The insight is one fact in `_aitna/memory/` (indexed); recurring facts have become a
