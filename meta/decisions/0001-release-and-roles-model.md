...
> This is akmon's **first ADR** and therefore also establishes the convention:
> `_aitna/akmon/decisions/` holds ADRs for the **standard itself** (SHARED, travels with the
...
akmon is one submodule consumed by N projects; bumping the pin is a recurring operation, and
...
commits*, and so akmon maintainers can cut a release by one repeatable pipeline — all without
violating D5.
...
3. **Anti-super-role invariant (routing, not doing).** `release` coordinates; it does not
   ...
   It never edits ADRs, production code, or `_aitna/memory/` in-cycle.
...
5. **`release` is parameterized by subject, not split into variants.** Its first gate selects one
   **subject**: *this project's package*, *akmon* (its tag), or *an akmon pin bump recorded
   inside a consuming project*. One role definition in akmon, incarnated per subject; a
...
6. **Subject relativity of the Layer axis.** DEVELOP / OPERATE / USAGE are **Layer-axis
   ...
   relative to *akmon* (consuming the mounted standard). The two consumption relations differ by
   **consequence**: USAGE consumption is build/dev-time and reversible (using a library; following
   akmon); OPERATE consumption is runtime with real, often irreversible consequences (orders,
   money) and is reserved for a runtime actor (ROADMAP O1, out of scope). Mounting akmon is
...
8. **One job per artifact** (no duplication): `TASKS.md` = live queue; `TASKS_ARCHIVE.md` =
   ...
   *consuming* project, not in akmon's release notes.
...
- Two new role definitions ship in akmon (`roles/release.md`, `roles/learn.md`) plus
...
- The learn loop gains an owning role; existing learn pipelines are wired under it rather than
  floating.
- akmon gains `CHANGELOG.md`, a `decisions/` ADR convention (this file), and a small
...
- Consuming projects inherit the release/learn roles and the subject-parameterized release flow
  when they bump their pin; each may add `_aitna/agents/{release,learn}/README.md` incarnations.
